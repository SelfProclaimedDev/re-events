...
functions.firestore
.document('followers/{userUid}/userFollowing/{profileId}')
.onDelete(async(snapshot, context) => {

    const batch = db.batch();

    batch.delete(db.collection('following').doc(context.params.profileId).collection('userFollowers').doc(context.params.userId));
    batch(db.collection('users').doc(context.params.profileId),{
        followerCount : firebase.firestore.FieldValue.increment(-1)
    })
